# from django.contrib.auth.decorators import login_required
from django.shortcuts import redirect
from django.contrib.auth.forms import UserChangeForm
from .models import user
from . serializers import userSerializer
from django.core.mail import send_mail
from .models import Activity

from django.shortcuts import render
from rest_framework.views import APIView
from rest_framework.response import Response


send_mail(
    'Subject here',
    'Here is the message.',
    'from@example.com',
    ['to@example.com'],
    fail_silently=True,
)


"""""
from django.shortcuts import render
from FMT.subscriptions.models import SubscriptionUser
from FMT.subscriptions.forms import SubscriptionUserSignUpForm


def subscription_signup(request):
    form = SubscriptionUserSignUpForm(request.POST or None)

    if form.is_valid():
        instance = form.save(commit=False)
        if SubscriptionUser.objects.filter(email=instance.email).exists():
            print("Sorry! This email already exist")
        else:
            instance.save()

    context = {

        'form': form,
    }
    template = "Personnel/sign_up.html"
    return render(request, template, context)


def subscription_unsubscribe(request):
    form = SubscriptionUserSignUpForm(request.POST or None)

    if form.is_valid():
        instance = form.save(commit=False)
        if SubscriptionUser.objects.filter(email=instance.email).exists():
            SubscriptionUser.objects.filter(email=instance.email).delete()

        else:
            print('Sorry but we did not find your email address')

    context = {
        'form': form,
    }
    template = "Personnel/unsubscribe.html"
    return render(request, template, context) """""
from django.shortcuts import render
from .models import Activity
from django.db.models import Avg, Count, Max, Min, Sum
from django.http import JsonResponse
from django.views.generic import View
from .forms import QueryForm
from rest_framework.views import APIView
from rest_framework.response import Response


# Create your views here.

def activities(request):
        '''Renders a search form if receiving a GET request, or renders a results
        page if receiving a POST request. Uses form data and data captured from
        calls to helper functions in order to send dynamic data to the template.'''

        if request.method == 'POST':
            query_form = QueryForm(request.POST)

            if query_form.is_valid():
                clean_query = query_form.cleaned_data

                type = clean_query['type']
                retailer = clean_query['retailer']
                start_date = clean_query['start_date']
                end_date = clean_query['end_date']

                # Call helper functions to obtain values to pass to template
                global start_date_str
                start_date_str = helper_start_date(start_date)

                global end_date_str
                end_date_str = helper_end_date(end_date)

                min_date_str = helper_min_date()
                max_date_str = helper_max_date()
                total_hhs = helper_total_hhs(min_date_str, max_date_str)
                count_hhs_for_query = count_hhs(type, start_date_str, end_date_str, retailer)
                type_list = helper_get_types(start_date_str, end_date_str)
                avg_type_spend_hh = top_buying_type(type_list, start_date_str, end_date_str)
                sorted_type_list = sorted(avg_type_spend_hh.keys())

                sorted_avg_spend = []
                for item in sorted_type_list:
                    sorted_avg_spend.append(avg_type_spend_hh[item][0])

                return render(request, 'Activities/results.html', {'type': type,
                                                                    'retailer': retailer,
                                                                    'start_date': start_date_str,
                                                                    'end_date': end_date_str,
                                                                    'min_date': min_date_str,
                                                                    'max_date': max_date_str,
                                                                    'total_hhs': total_hhs,
                                                                    'count_hhs': count_hhs_for_query,
                                                                    'type_list': type_list,
                                                                    'avg_type_spend_hh': avg_type_spend_hh,
                                                                    'sorted_type_list': sorted_type_list,
                                                                    'sorted_avg_spend': sorted_avg_spend})

        else:
            query_form = QueryForm()
            return render(request, 'Activities/activities.html', {'query_form': query_form})


def results(request):
    return render(request, 'Activities/results.html')


class BarChartData(APIView):

        authentication_classes = []
        permission_classes = []

        def get(self, request, format=None):
            '''Creates a list of types sorted alphabetically. Makes use of the
            global variables start_date_str and end_date_str as defined in the
            home view. Creates a list of average spend per household for each type
            during a time range, sorted to match the type order in sorted_type_list.'''

            all_types_qs = Activity.objects.filter(date__gte=start_date_str,
                                            date__lte=end_date_str).values('type').distinct()
            type_list = []

            for type in all_types_qs:
                type_list.append(type['type'])

            sorted_type_list = sorted(type_list)

            # Create a list of average spending per household that is sorted to match
            # the order the types appear in sorted_type_list
            avg_type_spend_hh = top_buying_type(sorted_type_list, start_date_str, end_date_str)

            sorted_avg_spend = []
            for type in sorted_type_list:
                sorted_avg_spend.append(avg_type_spend_hh[type][0])
            print(sorted_avg_spend)

            data = {'sorted_types': sorted_type_list,
                    'sorted_avg_spend': sorted_avg_spend}
            return Response(data)


    # Send JSON data to Chart.js via a REST API call
class PieChartData(APIView):

        authentication_classes = []
        permission_classes = []

        def get(self, request, format=None):
            '''Creates a list of types sorted alphabetically. Makes use of the
            global variables start_date_str and end_date_str as defined in the
            home view. Creates a list of items representing the number of households
            that purchased each type. The list is sorted to match the order the
            types appearing in sorted_type_list.'''

            all_types_qs = Activity.objects.filter(date__gte=start_date_str,
                                            date__lte=end_date_str).values('type').distinct()
            type_list = []

            for type in all_types_qs:
                type_list.append(type['type'])

            sorted_type_list = sorted(type_list)

            # Create a list of items representing the number of households that purchased
            # each type. The list is sorted to match the order the types appearing in sorted_type_list

            sorted_hhs_by_type = []

            for type in sorted_type_list:
                temp_hhs_by_type = Activity.objects.filter(date__gte=start_date_str,
                                    date__lte=end_date_str, type=type).values('user_id').distinct().count()
                print("Temp_hhs_by_type: ", temp_hhs_by_type)
                sorted_hhs_by_type.append(temp_hhs_by_type)
            print("Sorted hhs by type: ", sorted_hhs_by_type)

            data = {'sorted_types': sorted_type_list,
                    'sorted_hhs_by_type': sorted_hhs_by_type}

            return Response(data)

    # ------------ BEGIN HELPER FUNCTIONS ------------ #

def helper_start_date(start_date):
        '''Takes in a datetime object, which may either be typecast from a user-supplied
        date string or the earliest date found in the data table. Returns the date as a string.'''

        if not start_date:
            start_date_obj = Activity.objects.all().aggregate(Min('date'))
            start_date = start_date_obj['date__min']
        else:
            start_date = start_date

        return(start_date)


def helper_end_date(end_date):
        '''Takes in a datetime object, which may either be typecast from a user-supplied
        date string or the latest date found in the data table. Returns the date as a string.'''

        if not end_date:
            end_date_obj = Activity.objects.all().aggregate(Max('date'))
            end_date = end_date_obj['date__max']
        else:
            end_date = end_date

        return(end_date)


def helper_min_date():
        '''Does not require an input. Returns the earliest date in the data table.'''

        min_date_obj = Activity.objects.all().aggregate(Min('date'))
        min_date_str = min_date_obj['date__min']

        return(min_date_str)


def helper_max_date():
        '''Does not require an input. Returns the latest date in the data table.'''

        max_date_obj = Activity.objects.all().aggregate(Max('date'))
        max_date_str = max_date_obj['date__max']

        return(max_date_str)


def helper_total_hhs(min_date, max_date):
        '''Takes in the earliest and latest dates found in the data table.
        Returns the count of distinct households that shopped during that period.'''

        total_hhs = Activity.objects.values('user_id').distinct().count()
        total_hhs = "{:,}".format(total_hhs)
        return(total_hhs)


def helper_get_types(start_date, end_date):
        '''Takes in a start date and an end date. If dates are not
        supplied by the user, the earliest and latest dates in the
        table are used. Returns a list of types purchased within
        the dates specified.'''

        all_types_qs = Activity.objects.filter(date__gte=start_date,
                                            date__lte=end_date).values('type').distinct()
        type_list = []

        for type in all_types_qs:
            type_list.append(type['type'])

        return(type_list)

    # ------------ END HELPER FUNCTIONS ------------ #

    # ------------ BEGIN CALCULATION FUNCTIONS ------------ #


def count_hhs(type, start_date, end_date, retailer):
        '''Takes in a type (selected by user), start and end dates (selected by user,
        or the largest available timeframe by default), and a retailer (user specifies
        one or all). Returns the number of households that purchased that type within
        the timeframe given at the specified retailer(s).'''

        if retailer == 'all retailers':
            count_hhs = Activity.objects.filter(type=type, date__gte=start_date,
                                            date__lte=end_date).values('user_id').distinct().count()
            count_hhs = "{:,}".format(count_hhs)
            return(count_hhs)
        else:
            count_hhs = Activity.objects.filter(type=type,
                                            date__gte=start_date,
                                            date__lte=end_date,
                                            retailer=retailer).values('user_id').distinct().count()
            count_hhs = "{:,}".format(count_hhs)
            return(count_hhs)


def top_buying_type(types, start_date, end_date):
        '''Takes in a list of all types in the data table,
        and start and end dates. Dates are selected by the user,
        or the largest available timeframe is used by default.
        Returns the average dollars spent per household on each
        type within the timeframe given.'''

        avg_spend_hh_by_type = {}

        for type in types:

            sum_item_spend = Activity.objects.values('type').filter(date__gte=start_date, date__lte=end_date, type=type).annotate(total_spent=Sum('item_spend'))

            sum_hhs = Activity.objects.filter(type=type, date__gte=start_date,   date__lte=end_date).values('user_id').distinct().count()

            avg_spend_hh = round((sum_item_spend[0]['total_spent'] / sum_hhs), 2)

            avg_spend_hh_by_type[type] = [avg_spend_hh,
                                            "{:,}".format(sum_item_spend[0]['total_spent']),
                                            "{:,}".format(sum_hhs)]

        print(avg_spend_hh_by_type)

        return (avg_spend_hh_by_type)

# ------------ END CALCULATION FUNCTIONS ------------ #


class userList(APIView):

    def get(self, request):
        user1 = user.objects.all()
        serializer = userSerializer(user, many=True)
        return Response(serializer.data)

    def post(self):
        pass


def home(request):
    return render(request, 'Personnel/home.html')


def login(request):
    return render(request, 'Personnel/login.html')


def view_map(request):
    return render(request, 'Personnel/view_map.html')


def activities_stat(request):
    return render(request, 'Personnel/activities_stat.html')


def profile(request):
    args = {'user': request.user}
    return render(request, 'Personnel/profile.html', args)


def edit_profile(request):
    if request.method == 'POST':
        form = UserChangeForm(request.POST, instance=request.user)

        if form.is_valid():
            form.save()
            return redirect('Personnel:profile')
        else:
            form = UserChangeForm(instance=request.user)
            args = {'form': form}
            return render(request, 'Personnel:edit_profile.html', args)








